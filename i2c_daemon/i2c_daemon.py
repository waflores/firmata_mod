...
def main():
    ...
    parser.add_argument(
        ...
        "-l",
        action="store",
        help="Write logs to log_file. Default: [%(default)s]",
        default="/tmp/tmpdirwatch.log",
    )
    subparsers = parser.add_subparsers(dest="subcommand")

    # Do polling or inotify; for polling, how many seconds?
    watcher_type_parser = subparsers.add_parser(
        "poll", help="Use polling method to monitor file system events"
    )
    watcher_type_parser.add_argument(
        "--polling-interval",
        "-p",
        type=int,
        help="Number of seconds to wait between polling events",
        default=5,
    )
