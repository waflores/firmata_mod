...
static void firmata_ldisc_tx_worker(struct work_struct *work) {
  struct firmata_priv *firmata =
      container_of(work, struct firmata_priv, tx_work);
  ssize_t written;
...
// FIXME: This needs to be aligned with the message sizes in spi/i2c and the
// overall buffer
...
static void parse_sysex(struct firmata_priv *firmata, int len) {
  ...
  case REPORT_FIRMWARE:
    ...
    while ((firmata->rxbuf[i] != END_SYSEX) && (i / 2 < MSG_BUFFER_SIZE)) {
      buf[i / 2 - 2] =
          (firmata->rxbuf[i] & 0x7F) | ((firmata->rxbuf[i + 1] & 0x7F) << 7);
      i += 2;
    }
    buf[i / 2 - 2] = '\0';
  ...
  case PIN_STATE_RESPONSE:
    run_event_callbacks(firmata, SYSEX_ID | sysex_code, firmata->rxbuf + i,
                        len);
    break;
  case END_SYSEX:
...
/*
 * The function must only be called if there is at least one valid response byte
 * in rxbuf returns 0 if no more messages in buffer returns 1 if there is
 * possibly another message in the buffer
 */
static int parse_rxbuf(struct firmata_priv *firmata) {
  size_t pos = 0;

  pr_info("parse_rxbuf, fill %d, Message %02x %02x %02x\n", firmata->rxfill,
          firmata->rxbuf[pos], firmata->rxbuf[pos + 1],
          firmata->rxbuf[pos + 2]);
  lockdep_assert_held(&firmata->lock);
  ...
  if (firmata->rxbuf[pos] == START_SYSEX) {
    ...
    // Message is longer than buffer, assume some transmission problem and drop
    // everything
...
static size_t firmata_ldisc_rx(struct tty_struct *tty, const u8 *cp,
  ...
  if (received >= 0) {
    dev_err(firmata->tty->dev,
            "Receive buffer overflowed. Bad chip or wiring? received = %i",
            received);
...
static struct tty_ldisc_ops firmata_ldisc_ops = {
    //	.owner		= THIS_MODULE TODO: For now, so we can test, need to
    // split modules later
    .num = N_FIRMATA,
    .name = "firmata_ldisc",
...
static struct tty_struct *firmata_tty_init(struct device *dev) {
  ...
  pr_info(
      "Setting dev drvdata to firmata line discipline, firmata pointer %p\n",
      (void *)(tty->disc_data));
...
static int firmata_probe(struct platform_device *pdev) {
  ...
  spin_lock_bh(&firmata->lock);
  serial_send(tty, get_firmware_version,
              3); // TODO: This needs to come from the driver data
  ...
  err = mfd_add_hotplug_devices(&pdev->dev, firmata_privs,
                                ARRAY_SIZE(firmata_privs));
  if (err) {
...
static int firmata_suspend(struct device *pdev) { return 0; }

static int firmata_resume(struct device *pdev) { return 0; }
...
static void __exit firmata_exit(void) {
  ...
  // TODO: Instead of using the global firmata_tty, we should make use only of
  // getting the LDISC and the go for the firmata structure
