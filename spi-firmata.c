 ...
 * peripheral devices of the microcontroller available through a UART
 * to the host
 */
...
static int spi_reply_cb(struct platform_device *pdev, const u8 rxbuf[],
                        int len) {
  struct firmata_spi *spi = platform_get_drvdata(pdev);
  uint8_t id, channel;
  int i;

  pr_info("spi_reply_cb: received %02x %02x %02x %02x %02x %02x %02x %02x %02x "
          "%02x %02x %02x, len %d\n",
          rxbuf[0], rxbuf[1], rxbuf[2], rxbuf[3], rxbuf[4], rxbuf[5], rxbuf[6],
          rxbuf[7], rxbuf[8], rxbuf[9], rxbuf[10], rxbuf[11], len);

  // Discard any data sent outside an SPI transaction, e.g. spurious data on
  // serial interface
  ...
  if (rxbuf[0] != SPI_DATA) {
    pr_err("firmata spi_reply_cb: expected SPI_DATA message, got message code "
           "0x%02x\n",
           rxbuf[0]);
    return -1;
  }
  ...
  if (spi->req_id != rxbuf[3]) {
    pr_err("firmata spi_reply_cb: request-id mismatch, expecting %d, got %d",
           spi->req_id, rxbuf[4]);
    return -EPROTO;
  }
  if ((len - 6) / 2 != rxbuf[4]) {
...
static void send_dev_config(struct firmata_spi *spi,
                            struct spi_device *spi_dev) {
  struct firmata_platform_data *pdata = dev_get_platdata(&spi->pdev->dev);
  uint8_t buf[14];
  u8 id = 0;
  uint32_t speed =
      spi_dev->max_speed_hz; // TODO: use minimum of spi->speed and speed

  buf[0] = START_SYSEX;
  buf[1] = SPI_DATA;
  buf[2] = SPI_DEVICE_CONFIG;
  buf[3] = spi_get_chipselect(spi_dev, 0) | id << 3;
  buf[4] = ((spi_dev->mode & 0x3) << 1) |
           (spi_dev->mode & SPI_LSB_FIRST ? 0 : BIT(0));
  buf[5] = speed & 0x7f;
  buf[6] = (speed >> 7) & 0x7f;
  buf[7] = (speed >> 15) & 0x7f;
  buf[8] = (speed >> 22) & 0x7f;
  buf[9] = (speed >> 29) & 0x7f;
  // Be compatible with implementations which only support the default (0 -> 8
  // bit)
  buf[10] = spi_dev->bits_per_word != 8 ? spi_dev->bits_per_word : 0;
  buf[11] = (spi_dev->mode & SPI_CS_HIGH   ? BIT(1)
             : 0 | pdata->host_controls_cs ? 0
                                           : BIT(0));
  pr_info("send_dev_config: chip_select %d, pin %d\n",
          spi_get_chipselect(spi_dev, 0),
          cs_channel_mapping[spi_get_chipselect(spi_dev, 0)]);
...
static int firmata_spi_tx(struct firmata_spi *spi, u8 deselect_cs,
...
static int firmata_transfer_one(struct spi_controller *ctrl,
                                struct spi_device *spi_dev,
  ...
  timeout =
      wait_for_completion_timeout(&spi->reply_received, msecs_to_jiffies(500));
  if (!timeout) {
...
static int spi_reset_cb(struct platform_device *pdev, const u8 rxbuf[],
                        int len) {
  struct firmata_spi *spi = platform_get_drvdata(pdev);
...
static int firmata_spi_resume(struct device *pdev) { return 0; }
...
MODULE_DESCRIPTION(
    "Driver for SPI master for Firmata firmware on Microcontrollers");
