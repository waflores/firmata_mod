 ...
 * the GPIO's of the microcontroller available through a UART
 * to the host
 */
...
static int request_pin_report(struct firmata_gpio *gpio, u32 pin_mask) {
  ...
  // Check whether the pin value is already reported on as it is part of a
  // reported port
  for (i = 0; i < 4; i++) {
  ...
  pr_info("request_pin_report: pin_mask 0x%08x, required_ports: %04x, "
          "reported_ports %04x\n",
  ...
  timeout =
      wait_for_completion_timeout(&gpio->pin_value_read, msecs_to_jiffies(10));
...
static int firmata_gpio_direction_output(struct gpio_chip *gc, unsigned int off,
                                         int val) {
...
static int firmata_gpio_direction_input(struct gpio_chip *gc,
                                        unsigned int off) {
  struct firmata_gpio *gpio = gpiochip_get_data(gc);
  uint8_t buf[3];
...
static int firmata_gpio_set(struct gpio_chip *gc, unsigned int off, int val) {
  struct firmata_gpio *gpio = gpiochip_get_data(gc);
  int port_num;
  uint8_t port_val;
  uint8_t buf[3];
  ...
  return firmata_serial_tx(gpio->pdev, buf, 3);
}

static int firmata_gpio_set_multiple(struct gpio_chip *gc, unsigned long *maskp,
                                     unsigned long *bitsp) {
  struct firmata_gpio *gpio = gpiochip_get_data(gc);
  uint32_t mask, val;
  int port_num;
  ...
  pr_info("firmata_gpio_set_multiple: maskp %08lx, bitsp %08lx, mask %08x, val "
          "%08x",
  ...
  for (port_num = 0; port_num < 3; port_num++) {
    ...
    retval += firmata_serial_tx(gpio->pdev, buf, 3);
  }
  return retval;
}
...
static int firmata_gpio_get_multiple(struct gpio_chip *gc, unsigned long *maskp,
                                     unsigned long *bitsp) {
  struct firmata_gpio *gpio = gpiochip_get_data(gc);
  uint32_t v;
...
static int firmata_gpio_get_direction(struct gpio_chip *gc,
                                      unsigned int offset) {
  struct firmata_gpio *gpio = gpiochip_get_data(gc);

  pr_info("firmata_gpio_get_direction called, cached pin-direction 0x%08x, "
          "offset %d\n",
...
static int gpio_pin_value_cb(struct platform_device *pdev, const u8 rxbuf[],
                             int len) {
...
static int gpio_pin_state_cb(struct platform_device *pdev, const u8 rxbuf[],
                             int len) {
  struct firmata_gpio *gpio = platform_get_drvdata(pdev);

  pr_info(
      "gpio_pin_state_cb: command 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x, len %d\n",
      rxbuf[0], rxbuf[1], rxbuf[2], rxbuf[3], rxbuf[4], len);
  if (rxbuf[0] != PIN_STATE_RESPONSE) {
    pr_err("gpio_pin_state_cb: expected PIN_STATE_RESPONSE message, got "
           "message code 0x%02x\n",
           rxbuf[0]);
    return -1;
  }
...
static void firmata_gpio_get_pin_state(struct firmata_gpio *gpio, u8 pin) {
  ...
  timeout =
      wait_for_completion_timeout(&gpio->pin_state_read, msecs_to_jiffies(50));
  if (!timeout)
...
static int firmata_gpio_probe(struct platform_device *pdev) {
  ...
  // FIXME:	struct firmata_platform_data *pdata =
  ...
  err = firmata_register_event_cb(pdev, SYSEX_ID | PIN_STATE_RESPONSE,
                                  gpio_pin_state_cb);
  if (err) {
    dev_err(dev, "failed to register event cb: %d\n", err);
    return err;
...
#ifdef CONFIG_PM
static int firmata_gpio_suspend(struct device *dev) { return 0; }

static int firmata_gpio_resume(struct device *pdev) { return 0; }
