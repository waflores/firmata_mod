...
struct firmata_i2c {
  ...
  u8 *buf;
  int buf_fill;
  // Since we have to sleep waiting for I2C_REPLYs, we must ensure only one
  // request is ongoing a time
  struct mutex lock;
  struct completion reply_received;
};

static int i2c_reply_cb(struct platform_device *pdev, const u8 rxbuf[],
                        int len) {
  ...
  pr_info(
      "i2c_reply_cb: firmata_i2c pointer %p, bytes %02x %02x %02x, len %d\n",
      (void *)i2c, rxbuf[0], rxbuf[1], rxbuf[2], len);

  if (rxbuf[0] != I2C_REPLY) {
    pr_err("firmata i2c_reply_cb: expected I2C_REPLY message, got message code "
           "0x%02x\n",
           rxbuf[0]);
    return -1;
  }
  ...
  addr =
      rxbuf[1] | (rxbuf[2] << 7); // TODO: 10 bit mode?? Transaction sequence???
  addr &= 0x7f;
...
static int i2c_read(struct firmata_i2c *i2c, u16 addr, u8 *data, u16 data_len) {
  ...
  timeout =
      wait_for_completion_timeout(&i2c->reply_received, msecs_to_jiffies(200));
  if (!timeout) {
...
static int i2c_write(struct firmata_i2c *i2c, u16 addr, u8 *data,
                     u16 data_len) {
  int i;
...
static int firmata_i2c_xfer(struct i2c_adapter *adapter, struct i2c_msg *msgs,
                            int num) {
